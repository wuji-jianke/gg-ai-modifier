...
enum SettingsPane { ai, tools, about }
...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  SettingsPane _activePane = SettingsPane.ai;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: ListView(
              key: PageStorageKey<String>('settings-${_activePane.name}'),
              primary: true,
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
              physics: const ClampingScrollPhysics(),
              keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag,
              children: _buildPaneChildren(),
            ),
          ),
        ],
      ),
  ...
  Widget _buildPaneSwitcher() {
    return Container(
      ...
      child: Wrap(
        ...
        runSpacing: 8,
        children: [
          _paneChip(SettingsPane.ai, 'AI 配置', Icons.smart_toy_outlined),
          _paneChip(SettingsPane.tools, '工具与权限', Icons.tune),
          _paneChip(SettingsPane.about, '关于', Icons.info_outline),
        ],
      ),
    );
  }

  Widget _paneChip(SettingsPane pane, String label, IconData icon) {
    final isSelected = _activePane == pane;
    return ChoiceChip(
      avatar: Icon(
        icon,
        size: 16,
  ...
  List<Widget> _buildPaneChildren() {
    return switch (_activePane) {
      SettingsPane.ai => _buildAiPaneChildren(),
      SettingsPane.tools => _buildToolsPaneChildren(),
      SettingsPane.about => _buildAboutPaneChildren(),
    };
  }

  List<Widget> _buildAiPaneChildren() {
    return [
      _buildSectionTitle('AI 接口'),
      const SizedBox(height: 12),
  ...
  List<Widget> _buildToolsPaneChildren() {
    return [
      _buildSectionTitle('工具与权限'),
      const SizedBox(height: 12),
      _buildOverlayCard(),
      const SizedBox(height: 16),
  ...
  List<Widget> _buildAboutPaneChildren() {
    return [
